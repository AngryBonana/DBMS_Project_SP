...
Dbms::Dbms(std::filesystem::path data_root)
...
Database& Dbms::require_database(const std::string& name) {
    ...
    if (it == databases_.end()) {
        throw std::runtime_error("Unknown database '" + name + "'");
    }
    return *it->second;
}
...
Database& Dbms::active_db() {
    if (active_.empty()) {
        throw std::runtime_error("No database selected (use USE first)");
    }
